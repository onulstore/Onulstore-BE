...
public class BrandController {
    ...
    public ResponseEntity<BrandDto.BrandResponse> addBrand(@RequestBody BrandDto.BrandRequest brandRequest) {
    ...
    public ResponseEntity<BrandDto.BrandResponse> updateBrand(
            @RequestBody BrandDto.UpdateRequest updateRequest, @ApiParam(required = true) @PathVariable Long brandId) {
        return ResponseEntity.ok(brandService.updateBrand(updateRequest, brandId));
    }

    @DeleteMapping("/{brandId}")
    @ApiOperation(value = "브랜드 삭제")
    public ResponseEntity<String> deleteBrand(@ApiParam(required = true) @PathVariable Long brandId) {
